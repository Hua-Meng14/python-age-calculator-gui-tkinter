...
class App:
    ...
    def run(self):
        ...
        # label for year in which use was born
        ...
        def checkYear():
            # simple mehtod to chekc the validity of a user input birth year
        ...
        def checkMonth():
            # simple mehtod to check the validity of a user input birth month
            self.statement.destroy()
            try:
        ...
        def checkDay():
            ...
            try:
                ...
                self.statement = tk.Label(text=f"{nameValue.get()}'s birth dat cannot parse to int.", font='courier 10', bg='lightblue')
                self.statement.grid(row=6, column=1, pady=15)
                return False
        ...
        def calculateAge():
            ...
            if checkYear() and checkMonth() and checkDay():
            ...
            self.button = tk.Button(text="Calculate Age", font='courier 12 bold',fg='white', bg='dodgerblue', command=calculateAge)
            self.button.grid(row=5, column=1)

            # inifite loop to run program
